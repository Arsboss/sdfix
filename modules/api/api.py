...
def api_middleware(app: FastAPI):
    ...
    try:
        if os.environ.get('HYSOSI_RICH_EXCEPTIONS', None) is not None:
...
class Api:
    def __init__(self, app: FastAPI, queue_lock: Lock):
        ...
        if shared.cmd_opts.api_server_stop:
            self.add_api_route("/sdapi/v1/server-kill", self.kill_li, methods=["POST"])
            self.add_api_route("/sdapi/v1/server-restart", self.restart_li, methods=["POST"])
            self.add_api_route("/sdapi/v1/server-stop", self.stop_li, methods=["POST"])
