...
def save_config_state(name):
    ...
    print(f"Saving backup of li/extension state to {filename}.")
    with open(filename, "w", encoding="utf-8") as f:
        json.dump(current_config_state, f, indent=4, ensure_ascii=False)
    config_states.list_config_states()
    new_value = next(iter(config_states.all_config_states.keys()), "Current")
    new_choices = ["Current"] + list(config_states.all_config_states.keys())
    return gr.Dropdown.update(value=new_value, choices=new_choices), f"<span>Saved current li/extension state to \"{filename}\"</span>"
...
def restore_config_state(confirmed, config_state_name, restore_type):
    ...
    print(f"*** Restoring li state from backup: {restore_type} ***")

    if restore_type == "extensions" or restore_type == "both":
        shared.opts.restore_config_state_file = config_state["filepath"]
        shared.opts.save(shared.config_filename)

    if restore_type == "li" or restore_type == "both":
        config_states.restore_li_config(config_state)
...
def update_config_states_table(state_name):
    ...
    try:
        li_remote = config_state["li"]["remote"] or ""
        li_branch = config_state["li"]["branch"]
        li_commit_hash = config_state["li"]["commit_hash"] or "<unknown>"
        li_commit_date = config_state["li"]["commit_date"]
        if li_commit_date:
            li_commit_date = time.asctime(time.gmtime(li_commit_date))
        else:
            li_commit_date = "<unknown>"

        remote = f"""<a href="{html.escape(li_remote)}" target="_blank">{html.escape(li_remote or '')}</a>"""
        commit_link = make_commit_link(li_commit_hash, li_remote)
        date_link = make_commit_link(li_commit_hash, li_remote, li_commit_date)

        current_li = config_states.get_li_config()

        style_remote = ""
        style_branch = ""
        style_commit = ""
        if current_li["remote"] != li_remote:
            style_remote = STYLE_PRIMARY
        if current_li["branch"] != li_branch:
            style_branch = STYLE_PRIMARY
        if current_li["commit_hash"] != li_commit_hash:
...
<table id="config_state_li">
    ...
    <tbody>
        <tr>
            ...
            <td>
                <label{style_branch}>{li_branch}</label>
            </td>
            <td>
        ...
        with gr.Tabs(elem_id="tabs_extensions"):
            ...
            with gr.TabItem("Available", id="available"):
                with gr.Row():
                    ...
                    extensions_index_url = os.environ.get('HYSOSI_EXTENSIONS_INDEX', "https://raw.githubusercontent.com/huyebatik/stui-extensions/master/index.json")
            ...
            with gr.TabItem("Backup/Restore"):
                with gr.Row(elem_id="extensions_backup_top_row"):
                    ...
                    config_restore_type = gr.Radio(label="State to restore", choices=["extensions", "li", "both"], value="extensions", elem_id="extension_backup_restore_type")
